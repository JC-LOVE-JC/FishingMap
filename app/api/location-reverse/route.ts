...
export async function GET(request: NextRequest) {
  ...
  if (Number.isNaN(lat) || Number.isNaN(lng)) {
    return NextResponse.json({ result: null satisfies LocationSuggestion | null }, { status: 400 });
  }

  try {
    const result = MAPTILER_API_KEY
      ? await reverseMapTiler(lat, lng)
      : await reverseNominatim(lat, lng);

    return NextResponse.json({ result });
  } catch {
    return NextResponse.json({ result: null satisfies LocationSuggestion | null });
  }
}

async function reverseMapTiler(lat: number, lng: number) {
  ...
  if (!response.ok) {
    throw new Error("MapTiler reverse failed");
  }

  const payload = await response.json();
  return normalizeMapTilerFeature(payload.features?.[0]);
}
...
async function reverseNominatim(lat: number, lng: number) {
  ...
  const payload = await response.json();
  return normalizeNominatimResult(payload);
}

function normalizeMapTilerFeature(feature?: MapTilerFeature | null): LocationSuggestion | null {
  if (!feature) {
    return null;
  }
...
function normalizeNominatimResult(result?: NominatimResult | null): LocationSuggestion | null {
  if (!result) {
    return null;
  }
